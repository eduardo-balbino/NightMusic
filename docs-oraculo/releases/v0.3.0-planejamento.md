...
1. Criar `<typesDirectory>/config-schema.ts` (default: `src/tipos/config-schema.ts`) com types completos
...
- `<typesDirectory>/config-schema.ts` (novo) — default: `src/tipos/config-schema.ts`
...
if (framework) {
  console.log(`🔍 Framework detectado: ${framework.name} v${framework.version}`);
